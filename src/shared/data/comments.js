...
export async function listCommentsForCourse(courseId, { limit = 30 } = {}) {
  ...
  const res = await db.listDocuments(APPWRITE.databaseId, APPWRITE.collections.comments, [
...
export async function createComment({ courseId, lessonId = "", userId, body, parentId = "" }) {
  if (!hasAppwrite()) {
    const doc = {
  ...
  return db.createDocument(APPWRITE.databaseId, APPWRITE.collections.comments, ID.unique(), {
    courseId,
    lessonId,
    userId,
    body,
